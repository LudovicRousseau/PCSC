...
MSC_RV MSCListTokens( MSCULong32 listScope, MSCLPTokenInfo tokenArray, 
  ...
  readerLength = 0;
  tokensFound = 0; readerList = 0; strLoc = 0; i = 0;

  if ( arrayLength == 0 ) return MSC_INVALID_PARAMETER;
  ...
  for ( i=0; i < readerLength - 1; i++ ) {
    ...
    rv = SCardGetStatusChange( localHContext, INFINITE, &rgReaderStates, 1 );

    if ( pcscToMSC(rv) != MSC_SUCCESS ) {
      if (readerList) free(readerList);
      return pcscToMSC(rv);
    }      
	...
	if ( (tokensFound <= *arrayLength) && (tokenArray != 0) ) {
	  currentToken = &tokenArray[tokensFound-1];
	    ...
	    strncpy(currentToken->tokenName, MSC_TOKEN_EMPTY_STR,
		    MSC_MAXSIZE_TOKENAME);
	  } else if ( rv == 0 ) {
	    currentToken->tokenType |= MSC_TOKEN_TYPE_KNOWN;
	    strncpy(currentToken->tokenName, tokenInfo.tokenName, 
		    MSC_MAXSIZE_TOKENAME);
	  } else {
	    currentToken->tokenType |= MSC_TOKEN_TYPE_UNKNOWN;
	    strncpy(currentToken->tokenName, MSC_TOKEN_UNKNOWN_STR,
		    MSC_MAXSIZE_TOKENAME);
	  }

	  strncpy(currentToken->slotName, rgReaderStates.szReader,
                  MAX_READERNAME);

	  if ( rgReaderStates.dwEventState & SCARD_STATE_PRESENT ) {
	    memcpy(currentToken->tokenId, rgReaderStates.rgbAtr,
		   rgReaderStates.cbAtr);
...
MSC_RV MSCEstablishConnection( MSCLPTokenInfo tokenStruct, 
  ...
  MSCPVoid32 vInitFunction; MSCPVoid32 vIdFunction;
  ...
  tokenSize = 0; tokenList = 0; tokenSize = 0; selectedIFD = -1;
  tokenIdLength = 0; slotState = 0; slotProtocol = 0;
  slotNameSize = sizeof(slotName); vIdFunction = 0; vInitFunction =0;

  if ( pConnection == NULL ) return MSC_INVALID_PARAMETER; 
  if ( tokenStruct == NULL ) return MSC_INVALID_PARAMETER;
  if ( nameSize > MSC_MAXSIZE_AID ) return MSC_INVALID_PARAMETER;
  ...
  if ( sharingMode != MSC_SHARE_DIRECT ) {
    if ( strcmp(tokenStruct->tokenName, 
		MSC_TOKEN_EMPTY_STR) == 0 ) {
  ...
  /* Call SCardStatus, make sure the card information matches
     if it does not return an error.  If it does, copy it   */
  ...
  if ( (sharingMode == MSC_SHARE_DIRECT) &&
       (slotState & SCARD_ABSENT) ) {

    /* They asked for direct mode and no card 
       is inserted so we are done with this   */
...
MSC_RV MSCWaitForTokenEvent( MSCLPTokenInfo tokenArray, MSCULong32 arraySize,
  ...
  /* Allocate array of SCARD_READERSTATE_A structures, set UNAWARE
     on all of the structures to get the current status and then
     send them to GetStatusChange for blocking event               */
...
MSC_RV MSCCallbackForTokenEvent( MSCLPTokenInfo tokenArray,
				 MSCULong32 arraySize, MSCCallBack callBack, 
				 MSCPVoid32 appData ) {
  MSCLPEventWaitInfo evlist;
  MSCULong32 curToken;
  pthread_t newThread;
...
MSC_RV MSCBeginTransaction( MSCLPTokenConnection pConnection ) {  
  ...
  if ( localHContext == 0 ) return MSC_INTERNAL_ERROR;

  while (1) {
    rv  = SCardBeginTransaction( pConnection->hCard );    
    ret = pcscToMSC(rv);

    if ( ret == MSC_SUCCESS ) break;
...
MSC_RV MSCGetCapabilities( MSCLPTokenConnection pConnection, MSCULong32 Tag,
  ...
  if ( vFunction != 0 ) {
    libMSCGetCapabilities = (MSCLong32 (*)(MSCLPTokenConnection, MSCULong32, 
					   MSCPUChar8, MSCPULong32)) vFunction;
...
MSC_RV MSCExtendedFeature( MSCLPTokenConnection pConnection, 
  ...
  if ( vFunction != 0 ) {
    libMSCExtendedFeature = (MSCLong32 (*)(MSCLPTokenConnection, MSCULong32, 
					   MSCPUChar8, MSCULong32, MSCPUChar8,
					   MSCPULong32)) vFunction;
    rv = (*libMSCExtendedFeature)(pConnection, extFeature, outData, outLength,
				  inData, inLength);
...
MSC_RV MSCGenerateKeys( MSCLPTokenConnection pConnection, MSCUChar8 prvKeyNum,
			MSCUChar8 pubKeyNum, MSCLPGenKeyParams pParams ) {
  MSCLong32 rv;
  MSCPVoid32 vFunction;
  MSCLong32 (*libMSCGenerateKeys)(MSCLPTokenConnection, MSCUChar8, MSCUChar8,
				  MSCLPGenKeyParams );
  ...
  if ( vFunction != 0 ) {
    libMSCGenerateKeys = (MSCLong32 (*)(MSCLPTokenConnection, 
					MSCUChar8, MSCUChar8, 
					MSCLPGenKeyParams)) vFunction;
    rv = (*libMSCGenerateKeys)(pConnection, prvKeyNum, pubKeyNum, pParams);
...
MSC_RV MSCImportKey( MSCLPTokenConnection pConnection, MSCUChar8 keyNum,
  ...
  MSCLong32 rv;
  MSCPVoid32 vFunction;
  MSCLong32 (*libMSCImportKey)(MSCLPTokenConnection, MSCUChar8, MSCLPKeyACL, 
			       MSCPUChar8, MSCULong32, MSCLPKeyPolicy, 
  ...
  if ( vFunction != 0 ) {
    libMSCImportKey = (MSCLong32 (*)(MSCLPTokenConnection, MSCUChar8, 
				     ...
				     MSCLPKeyPolicy, MSCPVoid32, 
				     MSCUChar8)) vFunction;
    rv = (*libMSCImportKey)(pConnection, keyNum, pKeyACL, pKeyBlob, 
			    keyBlobSize, keyPolicy,
			    pAddParams, addParamsSize);
...
MSC_RV MSCExportKey( MSCLPTokenConnection pConnection, MSCUChar8 keyNum,
  ...
  MSCLong32 rv;
  MSCPVoid32 vFunction;
  MSCLong32 (*libMSCExportKey)(MSCLPTokenConnection, MSCUChar8, MSCPUChar8, 
			       MSCPULong32, MSCPVoid32, MSCUChar8);
  ...
  if ( vFunction != 0 ) {
    libMSCExportKey = (MSCLong32 (*)(MSCLPTokenConnection, MSCUChar8, 
				     MSCPUChar8, MSCPULong32,
				     MSCPVoid32, MSCUChar8)) vFunction;
    rv = (*libMSCExportKey)(pConnection, keyNum, pKeyBlob, keyBlobSize,
			    pAddParams, addParamsSize);
...
MSC_RV MSCComputeCrypt( MSCLPTokenConnection pConnection,
  ...
  if ( vFunction != 0 ) {
    libMSCComputeCrypt = (MSCLong32 (*)(MSCLPTokenConnection, MSCLPCryptInit, 
					MSCPUChar8, MSCULong32, MSCPUChar8, 
					MSCPULong32)) vFunction;
...
MSC_RV MSCExtAuthenticate( MSCLPTokenConnection pConnection, MSCUChar8 keyNum,
  ...
  if ( vFunction != 0 ) {
    libMSCExtAuthenticate = (MSCLong32 (*)(MSCLPTokenConnection, MSCUChar8, 
					   MSCUChar8, MSCUChar8,
...
MSC_RV MSCListKeys( MSCLPTokenConnection pConnection, MSCUChar8 seqOption,
  ...
  MSCLong32 rv;
  MSCPVoid32 vFunction;
  MSCLong32 (*libMSCListKeys)(MSCLPTokenConnection, MSCUChar8, MSCLPKeyInfo);
...
MSC_RV MSCImportCertificate( MSCLPTokenConnection pConnection,
  ...
  MSCLong32 (*libMSCImportCertificate)(MSCLPTokenConnection, MSCULong32, 
				       ...
				       MSCULong32, MSCPVoid32, MSCULong32);
  ...
  if ( vFunction != 0 ) {
    ...
    rv = (*libMSCImportCertificate)(pConnection, certNumber, certInfo, 
				    certBlob, certBlobSize, pAddParams, 
				    addParamsSize);
...
MSC_RV MSCListCertificates( MSCLPTokenConnection pConnection,
  ...
  if ( vFunction != 0 ) {
    libMSCListCertificates = (MSCLong32 (*)
			       (MSCLPTokenConnection, MSCUChar8,
				MSCLPCertInfo)) vFunction;
    rv = (*libMSCListCertificates)(pConnection, seqOption,
				   pCertInfo);
...
MSC_RV MSCCreatePIN( MSCLPTokenConnection pConnection, MSCUChar8 pinNum,
  ...
  if ( vFunction != 0 ) {
    libMSCCreatePIN = (MSCLong32 (*)(MSCLPTokenConnection, MSCUChar8, 
				     MSCUChar8, MSCPUChar8,
				     MSCULong32, MSCPUChar8, 
				     MSCUChar8)) vFunction;
    rv = (*libMSCCreatePIN)(pConnection, pinNum, pinAttempts, pPinCode,
			    pinCodeSize, pUnblockCode, unblockCodeSize);
...
MSC_RV MSCChangePIN( MSCLPTokenConnection pConnection, MSCUChar8 pinNum,
  ...
  if ( vFunction != 0 ) {
    libMSCChangePIN = (MSCLong32 (*)(MSCLPTokenConnection, MSCUChar8, 
				     MSCPUChar8, MSCUChar8,
				     MSCPUChar8, MSCUChar8)) vFunction;
    rv = (*libMSCChangePIN)(pConnection, pinNum, pOldPinCode, oldPinCodeSize,
			    pNewPinCode, newPinCodeSize);
...
MSC_RV MSCCreateObject( MSCLPTokenConnection pConnection, MSCString objectID,
...
MSC_RV MSCDeleteObject( MSCLPTokenConnection pConnection, 
  ...
  MSCLong32 rv;
  MSCPVoid32 vFunction;
  MSCLong32 (*libMSCDeleteObject)(MSCLPTokenConnection, MSCString, MSCUChar8);
...
MSC_RV MSCListObjects( MSCLPTokenConnection pConnection, MSCUChar8 seqOption, 
...
MSC_RV MSCGetChallenge( MSCLPTokenConnection pConnection, MSCPUChar8 pSeed,
  ...
  if ( vFunction != 0 ) {
    ...
    rv = (*libMSCGetChallenge)(pConnection, pSeed, seedSize, pRandomData,
			       randomDataSize);
...
MSC_RV MSCReEstablishConnection(MSCLPTokenConnection pConnection) {
  ...
  vInitFunction = 0; vFinFunction = 0;
  ...
  if ( rv != SCARD_S_SUCCESS ) return pcscToMSC(rv);
