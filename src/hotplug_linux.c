...
LONG HPReadBundleValues() {
  ...
  while ( (currFP = readdir(hpDir)) != 0 ) {
     ...
     */
     ...
     sprintf(fullPath, "%s%s%s", PCSCLITE_HP_DROPDIR, currFP->d_name,
             "/Contents/Info.plist");
     ...
     rv = LCFBundleFindValueWithKey( fullPath, PCSCLITE_PRODKEY_NAME,
                                     keyValue );      
     if ( rv == 0 ) {
       bundleTracker[listCount].productID = strtol(keyValue, 0, 16);
     }       
     ...
     rv = LCFBundleFindValueWithKey( fullPath, PCSCLITE_LIBRKEY_NAME,
                                     keyValue );      
     if ( rv == 0 ) {
       sprintf(fullLibPath, "%s%s%s%s", PCSCLITE_HP_DROPDIR, currFP->d_name,
               "/Contents/Linux/", keyValue);
...
void HPEstablishUSBNotifications() {
  ...
  rv = 0; i = 0; fd = 0; ret = 0;
  usbDeviceStatus = 0; dir = 0; dirB = 0; entry = 0; entryB = 0;
  ...
  for (i=0; i < bundleSize; i++ ) {
    ...
    while ((entry = readdir(dir)) != 0) {
      ...
      if (!dirB) {
	DebugLogB("USB path seems to have disappeared %s", dirpath);
        closedir(dir);
        return;
      }
	...
	} else if ( usbDescriptor.idVendor == bundleTracker[i].manuID &&
		    usbDescriptor.idProduct == bundleTracker[i].productID &&
...
LONG HPSearchHotPluggables() {
  ...
  for (i=0; i < PCSCLITE_HP_MAX_DRIVERS; i++) {
    bundleTracker[i].plugStatus = 0; bundleTracker[i].productID = 0;
