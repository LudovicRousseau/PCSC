...
void RawDeviceAdded(void *refCon, io_iterator_t iterator)
{
    ...
    while (obj = IOIteratorNext(iterator)) {
      ...
      if ( refCon == NULL ) { /* Dont do this on (void *)1 */
...
void HPEstablishUSBNotifications() {
  ...
  for (i=0; i<bundleArraySize; i++) {
    ...
    propertyString = CFDictionaryGetValue( bundleInfoDict, 
					   CFSTR("ifdVendorID") );
    if ( propertyString == 0 ) {
      DebugLogA("HPEstablishUSBNotifications: No vendor id in bundle.");
      continue;
     }    
    ...
    propertyString = CFDictionaryGetValue( bundleInfoDict, 
					   CFSTR("ifdProductID") );
    if ( propertyString == 0 ) {
      DebugLogA("HPEstablishUSBNotifications: No product id in bundle.");
      continue;
     }    
    ...
    if (!bundleTracker[i].matchingDict) {
      DebugLogA("HPEstablishUSBNotifications: Can't make USBMatch dict.");
      continue;
     }    
    ...
    CFDictionarySetValue( 
            ...
            CFNumberCreate(kCFAllocatorDefault, kCFNumberSInt32Type, &hpManu_id)); 
    CFDictionarySetValue( 
            bundleTracker[i].matchingDict, 
            CFSTR(kUSBProductName), 
            CFNumberCreate(kCFAllocatorDefault, kCFNumberSInt32Type, &hpProd_id)); 
    ...
    bundleTracker[i].matchingDict = (CFMutableDictionaryRef) CFRetain( bundleTracker[i].matchingDict ); 
    bundleTracker[i].matchingDict = (CFMutableDictionaryRef) CFRetain( bundleTracker[i].matchingDict ); 
    bundleTracker[i].matchingDict = (CFMutableDictionaryRef) CFRetain( bundleTracker[i].matchingDict ); 
...
LONG HPSetupHotPlugDevice() {
  ...
  UInt32		addrHolder[PCSCLITE_HP_MAX_IDENTICAL_READERS];
   ...
   for (j=0; j < PCSCLITE_HP_MAX_IDENTICAL_READERS; j++) {
    ...
    if (kr) {
      DebugLogB("HPSearchHotPluggables: Can't make USBSvc iter: %x.", kr);
        if ( USBMatch )  CFRelease(USBMatch);
        return -1;
    }
  ...
  for (i=0; i<bundleArraySize; i++) {
    ...
    while ( (USBDevice = IOIteratorNext(iter)) )
    {
        ...
        if (kr || !iodev) {
	  DebugLogB("HPSearchHotPluggables: Can't make plugin intface: %x.", kr);
          SYS_Sleep(1);
          kr = IOCreatePlugInInterfaceForService(USBDevice, kIOUSBDeviceUserClientTypeID, 
                                                 kIOCFPlugInInterfaceID, &iodev, &score);
          if (kr || !iodev) {
        ...
        res = (*iodev)->QueryInterface(iodev, CFUUIDGetUUIDBytes(kIOUSBDeviceInterfaceID), 
                                       (LPVOID)&dev);
        IODestroyPlugInInterface(iodev);

        if (res || !dev) {
	  DebugLogB("HPSearchHotPluggables: Can't query interface: %x.", res);
          continue;
        }
    ...
    /* Look through the addresses of devices connected for one's
       that we have not seen yet, see if we must add anything 
    */
    ...
    for ( k=0; k < j; k++ ) {
      ...
      if ( x == PCSCLITE_HP_MAX_IDENTICAL_READERS ) {
        /* Here we will add the reader, since it is not in the address list */
    ...
    /* Look through the addresses of already connected devices for one's
       not seen on the subsystem, see if we must delete anything
    */
    ...
    for (x=0; x < PCSCLITE_HP_MAX_IDENTICAL_READERS; x++) {
      if ( bundleTracker[i].addrList[x] != 0 ) {
        ...
        if ( k == j ) {
          /* Here we will remove the reader, since it is not in the 
	     device address list */
