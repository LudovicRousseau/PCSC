...
LONG RFAllocateReaderSpace( DWORD dwAllocNum ) {
  ...
  /* Zero out everything */
  i=0; j=0; fd=0; psize=0;

  /* Here we must map the sContexts over to a memory map so when
     the server is threaded or forked we can continue to keep 
     track of the reader's state.
  */
  
  /* First open and lock the file to test for other running daemons */

  fd = SYS_OpenFile( PCSCLITE_SHM_FILE, O_RDWR | O_CREAT | O_NONBLOCK, 0666 );
  if ( fd < 0 ) {
...
LONG RFAddReader( LPSTR lpcReader, DWORD dwPort, LPSTR lpcLibrary ) {
  ...
  /* Zero out everything */
  dwContext=0; dwContextB=0; dwGetSize=0; rv=0; 
  i=0; j=0; psize=0; ucGetData[0]=0; tmplen=0;
  ...
  if ( dwNumContexts != 0 ) {
	...
	strncpy(lpcStripReader, (sContexts[i])->lpcReader, MAX_READERNAME);
...
LONG RFSetReaderName( PREADER_CONTEXT rContext, LPSTR readerName, 
  ...
  LONG rv;                       /* rv is the reader number of the 
				    parent of the clone */
  LONG ret;
  DWORD valueLength;
  UCHAR tagValue;
  ...
  if ( dwSlot == 0 ) {
    if ( *dwNumContexts != 0 ) {
      for ( i=0; i < PCSCLITE_MAX_CONTEXTS; i++ ) {
	if ( (sContexts[i])->vHandle != 0 ) {
	  if ( strcmp((sContexts[i])->lpcLibrary, libraryName) == 0 ) {
	    ...
	    if ( ((((sContexts[i])->dwPort & 0xFFFF0000) == 0x200000) &&
		 (sContexts[i])->dwPort != dwPort)|| 
		 (supportedChannels > 1) ) {
	      
              /* rv tells the caller who the parent of this clone is so  
                 it can use it's shared resources like mutex/etc.
              */

	      rv = i;	      

	      /* If the same reader already exists and it is hotplug then
		 we must look for others and enumerate the readername */

	      currentDigit = (sContexts[i])->
		lpcReader[strlen((sContexts[i])->lpcReader) - 3] - '0';
	...
	return -1;
      ...
      if ( i <= 9 ) {
	sprintf(rContext->lpcReader, "%s %d %ld", readerName, i, dwSlot);
      } else {
	sprintf(rContext->lpcReader, "%s %c %ld", readerName, 'A'+(i%10), dwSlot);
      }
    ...
    /* On the second, third slot of the reader use the last
       used reader number 
    */
...
LONG RFListReaders( LPSTR lpcReaders, LPDWORD pdwReaderNum ) {
  ...
  /* Zero out everything */
  dwCSize = 0; lpcTReaders = 0; i=0; p=0;
...
LONG RFBindFunctions( PREADER_CONTEXT rContext ) {
  ...
  /* Use this function as a dummy to determine the IFD
     Handler version type.  1.0/2.0.  Suppress error
     messaging since it can't be 1.0 and 2.0.
  */

  DebugLogSuppress( DEBUGLOG_IGNORE_ENTRIES );

  rv = DYN_GetAddress( rContext->vHandle, 
		       &rContext->psFunctions.pvfCreateChannel, 
		       "IO_Create_Channel" );
  ...
  if ( rContext->dwVersion & IFD_HVERSION_1_0 ) {
    ...
    rv = DYN_GetAddress( rContext->vHandle, 
			 &rContext->psFunctions.pvfCloseChannel, 
			 "IO_Close_Channel" );
    ...
    rv = DYN_GetAddress( rContext->vHandle, 
			 &rContext->psFunctions.pvfPowerICC, 
			 "IFD_Power_ICC" );
    ...
    rv = DYN_GetAddress( rContext->vHandle, 
			 &rContext->psFunctions.pvfSwallowICC, 
			 "IFD_Swallow_ICC" );
    ...
    rv = DYN_GetAddress( rContext->vHandle, 
			 &rContext->psFunctions.pvfEjectICC, 
			 "IFD_Eject_ICC" );
    ...
    rv = DYN_GetAddress( rContext->vHandle, 
			 &rContext->psFunctions.pvfConfiscateICC, 
			 "IFD_Confiscate_ICC" );
    ...
    rv = DYN_GetAddress( rContext->vHandle, 
			 &rContext->psFunctions.pvfTransmitICC, 
			 "IFD_Transmit_to_ICC" );
    ...
    rv = DYN_GetAddress( rContext->vHandle, 
			 &rContext->psFunctions.pvfICCPresent, 
			 "IFD_Is_ICC_Present" );
    ...
    rv = DYN_GetAddress( rContext->vHandle, 
			 &rContext->psFunctions.pvfICCAbsent, 
			 "IFD_Is_ICC_Absent" );
  ...
  /* The following binds version 2.0 of the IFD Handler specs */
    ...
    rv = DYN_GetAddress( rContext->vHandle, 
			 &rContext->psFunctions.pvfControl, 
			 "IFDHControl" );
...
LONG RFInitializeReader( PREADER_CONTEXT rContext ) {
  ...
  if ( rv != IFD_SUCCESS ) { 
    DebugLogB("RFInitializeReader: Open Port %X Failed", rContext->dwPort);
...
LONG RFUnInitializeReader( PREADER_CONTEXT rContext ) { 
  ...
  /*  If the reader is getting uninitialized then it is being 
      unplugged so I can't send a IFDPowerICC call to it
...
SCARDHANDLE RFCreateReaderHandle( PREADER_CONTEXT rContext ) {
  ...
  /* Zero out everything */
  randHandle=0; i=0; j=0;

  /* Create a random handle with 16 bits
     check to see if it already is used.
  */
  ...
  while (1) {
    ...
    /* Once the for loop is completed w/o restart a
       good handle was found and the loop can be exited. */
...
LONG RFAddReaderHandle( PREADER_CONTEXT rContext, SCARDHANDLE hCard ) {
  ...
  if ( i == PCSCLITE_MAX_CONTEXTS ) {     /* List is full */
    return SCARD_E_INSUFFICIENT_BUFFER;
  }
...
LONG RFRemoveReaderHandle( PREADER_CONTEXT rContext, SCARDHANDLE hCard ) {
  ...
  if ( i == PCSCLITE_MAX_CONTEXTS ) {     /* Not Found */
    return SCARD_E_INVALID_HANDLE;
  }
...
void RFCleanupReaders(void)
{
	...
	for (i=0; i<PCSCLITE_MAX_CONTEXTS; i++)
	{
		if (sContexts[i] -> vHandle != 0)
		{
			...
			if (rv != SCARD_S_SUCCESS)
				DebugLogB("RFRemoveReader error: %s", pcsc_stringify_error(rv));
		}
	}
