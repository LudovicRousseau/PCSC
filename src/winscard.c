...
LONG SCardEstablishContext( DWORD dwScope, LPCVOID pvReserved1, 
  ...
  /* Unique identifier for this server so that it can uniquely 
     be identified by clients and distinguished from others    */


  *phContext = (PCSCLITE_SVC_IDENTITY + SYS_Random(SYS_GetSeed(), 
                1.0, 65535.0));                                                             

  DebugLogB("SCardEstablishContext: Establishing Context: %d", *phContext);
...
LONG SCardConnect( SCARDCONTEXT hContext, LPCSTR szReader, DWORD dwShareMode,
  ...
  /* Zero out everything */
  rv=0; rContext=0; ucAvailable=0;
  dwAtrLength=0; dwState=0; dwStatus=0; dwReaderLen=0; 
  dwProtocol=0; memset( pucAtr, 0x00, MAX_ATR_SIZE );
...
LONG SCardReconnect( SCARDHANDLE hCard, DWORD dwShareMode,
  ...
  if ( dwPreferredProtocols & SCARD_PROTOCOL_RAW ) {
    ...
    if ( dwShareMode != SCARD_SHARE_DIRECT ) {
      ...
      /* If it is set to any let it do any of the protocols */
      if ( dwPreferredProtocols & SCARD_PROTOCOL_ANY ) {
	rContext->dwProtocol = PHSetProtocol( rContext, SCARD_PROTOCOL_T0 |
					      SCARD_PROTOCOL_T1, ucAvailable );
      } else {
	rContext->dwProtocol = PHSetProtocol( rContext, dwPreferredProtocols,
					      ucAvailable );
	if (rContext->dwProtocol == -1) {
...
LONG SCardDisconnect( SCARDHANDLE hCard, DWORD dwDisposition ) {
  ...
  /* Zero out everything */
  rv=0; rContext=0; dwAction=0; dwAtrLen=0; receiveLength=0;
  ...
  DebugLogB("SCardDisconnect: Active Contexts: %d", rContext->dwContexts);
  ...
  /*  if ( RFCheckSharing( hCard ) == SCARD_S_SUCCESS ) {  */
	...
	controlBuffer[0] = 0x20; controlBuffer[1] = 0x15;
	controlBuffer[2] = (rContext->dwSlot & 0x0000FFFF) + 1;
	controlBuffer[3] = 0x00; controlBuffer[4] = 0x00;
	receiveLength    = 2;
...
LONG SCardBeginTransaction( SCARDHANDLE hCard ) {
  ...
  if ( (rv = RFCheckReaderEventState( rContext, hCard )) 
       != SCARD_S_SUCCESS ) 
    {
      return rv;
    }
...
LONG SCardEndTransaction( SCARDHANDLE hCard, DWORD dwDisposition ) {
  ...
  /* Zero out everything */
  rContext=0; rv=0; dwAction=0; receiveLength=0;
  ...
  if ( dwDisposition == SCARD_RESET_CARD ||
	...
	if ( rv == SCARD_S_SUCCESS ) {
	  if ( receiveLength == 2 && receiveBuffer[0] == 0x90 ) {
	    DebugLogA("SCardEndTransaction: Card ejected successfully.");
	    /* Successful */
	  } else {
...
LONG SCardStatus( SCARDHANDLE hCard, LPSTR mszReaderNames, 
  ...
  /* This is a client side function however the server maintains
     the list of events between applications so it must be passed
     through to obtain this event if it has occurred
  */
...
LONG SCardTransmit( SCARDHANDLE hCard, LPCSCARD_IO_REQUEST pioSendPci,
  ...
  /* Zero out everything */
  rv=0; rContext=0; dwRxLength=0; tempRxLength=0;
  ...
  *pcbRecvLength = 0;
  ...
  /* Removed - a user may allocate a larger buffer
  ...
  */
  ...
  if ( pioSendPci->dwProtocol == SCARD_PROTOCOL_RAW ) {
    ...
    rv = IFDTransmit( rContext, sSendPci, (PUCHAR)pbSendBuffer, cbSendLength,
